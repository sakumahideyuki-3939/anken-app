...
├── logic_core.php         # 【編集禁止】算出ロジック本体（年星・月星・日星の計算）
├── setsu.php              # 節入り日テーブル（1927〜2032年の全データ）
...
## よくあるトラブル・注意点
- **`logic_core.php` は編集禁止**: ファイル冒頭に「絶対編集禁止」と明記されている。算出ロジックの心臓部であり、変更すると全判定が壊れる
...
- **setsu.phpの年範囲**: 1927〜2032年のデータのみ。範囲外の年を指定すると節入り補正が効かない
